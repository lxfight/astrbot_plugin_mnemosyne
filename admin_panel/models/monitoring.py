...
class ResourceUsage:
    ...
    def to_dict(self) -> dict:
        """转换为字典"""
        memory_usage_percent = (self.memory_used_mb / self.memory_limit_mb * 100) if self.memory_limit_mb else None
        
        return {
            "memory": {
                "used_mb": round(self.memory_used_mb, 2),
                "limit_mb": round(self.memory_limit_mb, 2) if self.memory_limit_mb else None,
                "usage_percent": round(memory_usage_percent, 2) if memory_usage_percent else None
            },
            "database": {
                "size_mb": round(self.db_size_mb, 2)
            },
            "vector_database": {
