...
from ..models.monitoring import (
    SystemStatus, ComponentHealth, ComponentStatus,
    PerformanceMetrics, ResourceUsage, BackgroundTaskStatus
)
...
class MetricsCollector:
    ...
    def get_metrics(self) -> PerformanceMetrics:
        ...
        if self.memory_query_times:
            metrics.memory_query_p50 = self._calculate_percentile(self.memory_query_times, 0.50)
            metrics.memory_query_p95 = self._calculate_percentile(self.memory_query_times, 0.95)
            metrics.memory_query_p99 = self._calculate_percentile(self.memory_query_times, 0.99)
        
        # 计算向量搜索性能
        if self.vector_search_times:
            metrics.vector_search_p50 = self._calculate_percentile(self.vector_search_times, 0.50)
            metrics.vector_search_p95 = self._calculate_percentile(self.vector_search_times, 0.95)
            metrics.vector_search_p99 = self._calculate_percentile(self.vector_search_times, 0.99)
        
        # 计算数据库操作性能
        if self.db_operation_times:
            metrics.db_operation_p50 = self._calculate_percentile(self.db_operation_times, 0.50)
            metrics.db_operation_p95 = self._calculate_percentile(self.db_operation_times, 0.95)
            metrics.db_operation_p99 = self._calculate_percentile(self.db_operation_times, 0.99)
        
        # 计算 API 成功率
        total_embedding = self.embedding_api_calls["success"] + self.embedding_api_calls["failed"]
        if total_embedding > 0:
            metrics.embedding_api_success_rate = (self.embedding_api_calls["success"] / total_embedding) * 100
        
        total_milvus = self.milvus_api_calls["success"] + self.milvus_api_calls["failed"]
        if total_milvus > 0:
            metrics.milvus_api_success_rate = (self.milvus_api_calls["success"] / total_milvus) * 100
...
class MonitoringService:
    ...
    async def get_system_status(self, force_refresh: bool = False) -> SystemStatus:
        ...
        if not force_refresh and self._last_health_check:
            cache_age = (datetime.now() - self._last_health_check.timestamp).total_seconds()
        ...
        if not overall_healthy:
            ...
            overall_status = ComponentStatus.UNHEALTHY if has_unhealthy else ComponentStatus.DEGRADED
        else:
    ...
    async def _check_message_counter_health(self) -> ComponentHealth:
        ...
        try:
            ...
            # 测试数据库连接
            test_count = self.plugin.msg_counter.get_counter("_health_check_")
    ...
    async def get_resource_usage(self) -> ResourceUsage:
        ...
        try:
            ...
            if self.plugin.milvus_manager and self.plugin.milvus_manager.is_connected():
                ...
                # 获取当前集合的记录数
                if self.plugin.milvus_manager.has_collection(self.plugin.collection_name):
                    collection = self.plugin.milvus_manager.get_collection(self.plugin.collection_name)
        ...
        try:
            ...
            if self.plugin.context_manager:
                ...
                now = time.time()
                active_count = 0
                for session_id, conv_data in self.plugin.context_manager.conversations.items():
        ...
        try:
            # 获取后台任务状态
            if hasattr(self.plugin, '_summary_check_task') and self.plugin._summary_check_task:
                task = self.plugin._summary_check_task
                if not task.done():
