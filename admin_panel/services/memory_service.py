...
class MemoryService:
    ...
    async def search_memories(self, request: MemorySearchRequest) -> MemorySearchResponse:
        ...
        """
        try:
            if not self.plugin.milvus_manager or not self.plugin.milvus_manager.is_connected():
            ...
            # 查询记忆
            output_fields = ["memory_id", "session_id", "content", "create_time", "persona_id"]
            ...
            try:
                ...
                # 获取总数（近似值）
                total_count = collection.num_entities if not expr else len(records) + request.offset
    ...
    async def get_memory_statistics(self) -> MemoryStatistics:
        ...
        try:
            ...
            if max_query > 0:
                ...
                stats.average_memory_length = total_length / len(results) if results else 0.0
    ...
    async def get_session_list(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for result in results:
                ...
                if session_info["last_memory_time"] is None or create_time > session_info["last_memory_time"]:
                    session_info["last_memory_time"] = create_time
                
                if session_info["first_memory_time"] is None or create_time < session_info["first_memory_time"]:
                    session_info["first_memory_time"] = create_time
            
            # 转换为列表并排序
            sessions = list(session_data.values())
            sessions.sort(
                key=lambda x: x["last_memory_time"] if x["last_memory_time"] else datetime.min,
                reverse=True
            )
            ...
            for session in sessions[:limit]:
                if session["last_memory_time"]:
                    session["last_memory_time"] = session["last_memory_time"].isoformat()
                if session["first_memory_time"]:
                    session["first_memory_time"] = session["first_memory_time"].isoformat()
