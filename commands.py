...
async def delete_collection_cmd_impl(
    ...
    try:
        ...
        self.logger.warning(f"管理员 {sender_id} 请求删除集合: {collection_name} (确认执行)")
        if is_current_collection:
            self.logger.critical(f"管理员 {sender_id} 正在删除当前插件使用的集合 '{collection_name}'！")
        ...
        if success:
            ...
            if is_current_collection:
                self.logger.error(f"插件当前使用的集合 '{collection_name}' 已被删除，相关功能将不可用。")
        else:
            yield event.plain_result(f"⚠️ 删除集合 '{collection_name}' 的请求已发送，但 Milvus 返回失败。请检查 Milvus 日志获取详细信息。")

    except Exception as e:
        self.logger.error(f"执行 'memory drop_collection {collection_name}' 命令时发生严重错误: {str(e)}", exc_info=True)
...
async def list_records_cmd_impl(
    ...
    try:
        ...
        if query_limit > 16384:
            yield event.plain_result(f"⚠️ 查询范围过大 (offset + limit = {query_limit})，超过 Milvus 限制。请减小数量或偏移量。")
            return

        expr = f"{PRIMARY_FIELD_NAME} >= 0"
        output_fields = ["content", "create_time", "session_id", "personality_id", PRIMARY_FIELD_NAME]

        self.logger.debug(f"查询集合 '{target_collection}' 记录: expr='{expr}', limit={query_limit}, output_fields={output_fields}")

        records = self.milvus_manager.query(
            collection_name=target_collection, expression=expr, output_fields=output_fields, limit=query_limit
        )

        if records is None:
            yield event.plain_result(f"⚠️ 查询集合 '{target_collection}' 记录失败，请检查日志。")
            return
        if not records:
            yield event.plain_result(f"集合 '{target_collection}' 中没有找到任何记忆记录。")
            return
        ...
        if not paginated_records:
            yield event.plain_result(f"在指定的偏移量 {offset} 之后，集合 '{target_collection}' 没有更多记录了。")
            return

        total_found_in_query = len(records)
        response_lines = [f"📜 集合 '{target_collection}' 的记忆记录 (显示第 {offset + 1} 到 {offset + len(paginated_records)} 条，按时间倒序):"]
        ...
        self.logger.error(f"执行 'memory list_records' 命令失败 (集合: {target_collection}): {str(e)}", exc_info=True)
...
async def delete_session_memory_cmd_impl(
    ...
    try:
        ...
        sender_id = event.get_sender_id()
        self.logger.warning(f"管理员 {sender_id} 请求删除会话 '{session_id_to_delete}' 的所有记忆 (集合: {collection_name}, 表达式: '{expr}') (确认执行)")

        mutation_result = self.milvus_manager.delete(collection_name=collection_name, expression=expr)

        if mutation_result:
            delete_pk_count = mutation_result.delete_count if hasattr(mutation_result, 'delete_count') else '未知'
            self.logger.info(f"已发送删除会话 '{session_id_to_delete}' 记忆的请求。返回的删除计数（可能不准确）: {delete_pk_count}")
            try:
                self.logger.info(f"正在刷新 (Flush) 集合 '{collection_name}' 以应用删除操作...")
                self.milvus_manager.flush([collection_name])
                self.logger.info(f"集合 '{collection_name}' 刷新完成。删除操作已生效。")
                yield event.plain_result(f"✅ 已成功删除会话 ID '{session_id_to_delete}' 的所有记忆信息。")
            except Exception as flush_err:
                self.logger.error(f"刷新集合 '{collection_name}' 以应用删除时出错: {flush_err}", exc_info=True)
                yield event.plain_result(f"⚠️ 已发送删除请求，但在刷新集合使更改生效时出错: {flush_err}。删除可能未完全生效。")
        else:
            yield event.plain_result(f"⚠️ 删除会话 ID '{session_id_to_delete}' 记忆的请求失败。请检查 Milvus 日志。")

    except Exception as e:
        self.logger.error(f"执行 'memory delete_session_memory' 命令时发生严重错误 (Session ID: {session_id_to_delete}): {str(e)}", exc_info=True)
...
async def get_session_id_cmd_impl(self: 'Mnemosyne', event: AstrMessageEvent):
    ...
    try:
        ...
        if session_id:
            ...
            yield event.plain_result("🤔 无法获取当前会话 ID。可能还没有开始对话，或者会话已结束/失效。")
            self.logger.warning(f"用户 {event.get_sender_id()} 在 {event.unified_msg_origin} 尝试获取 session_id 失败。")
    except Exception as e:
        self.logger.error(f"执行 'memory get_session_id' 命令失败: {str(e)}", exc_info=True)
