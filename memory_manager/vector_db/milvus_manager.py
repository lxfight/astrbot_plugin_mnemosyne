...
class MilvusManager:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            plugin_data_dir (Optional[str]): 插件数据目录路径，用于 Milvus Lite 模式。
            **kwargs: 传递给 connections.connect 的其他参数。
        """

        self.alias = alias
        self._plugin_data_dir = plugin_data_dir  # 存储插件数据目录
    ...
    def _prepare_lite_path(self, path_input: str) -> str:
        ...
        if not self._plugin_data_dir:
            # 如果没有传入插件数据目录，尝试使用 StarTools（这可能会失败）
            try:
                default_data_dir = pathlib.Path(StarTools.get_data_dir())
                logger.warning(
                    "未传入 plugin_data_dir 参数，尝试通过 StarTools.get_data_dir() 获取（可能失败）"
                )
            except Exception as e:
                logger.error(f"致命错误：无法获取插件数据目录: {e}")
                logger.error("请在创建 MilvusManager 时传入 plugin_data_dir 参数")
                raise RuntimeError(
                    f"无法初始化 Milvus Lite 路径：无法获取插件数据目录 - {e}"
                ) from e
        else:
            default_data_dir = pathlib.Path(self._plugin_data_dir)
            logger.debug(f"使用传入的插件数据目录: {default_data_dir}")
    ...
    def _get_default_lite_path(self) -> str:
        """计算默认的 Milvus Lite 数据路径。"""
        try:
            # 使用传入的插件数据目录
            if not self._plugin_data_dir:
                # 如果没有传入，尝试使用 StarTools（可能失败）
                try:
                    ...
                    logger.error(f"致命错误：无法获取插件数据目录: {e}")
                    logger.error("请在创建 MilvusManager 时传入 plugin_data_dir 参数")
                    raise RuntimeError(f"无法初始化默认 Milvus Lite 路径：{e}") from e
            else:
                default_dir = pathlib.Path(self._plugin_data_dir)
                logger.debug(f"使用传入的插件数据目录: {default_dir}")
            ...
            logger.error("Milvus Lite 必须使用插件数据目录，不允许使用硬编码或回退路径")
            raise RuntimeError(f"无法初始化默认 Milvus Lite 路径：{e}") from e
