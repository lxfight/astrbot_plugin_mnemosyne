...
class MilvusManager:
        ...
        **kwargs
        ...
        if self._uri:
            ...
            if self._secure is None and parsed_uri.scheme == 'https':
                logger.info(f"URI scheme 是 'https'，将设置 secure=True (别名: {self.alias})。")
                self._secure = True
            elif self._secure is None:
                self._secure = False # 默认不安全
            self._connection_info['secure'] = self._secure
        elif self._host:
            logger.info(f"将使用 Host '{self._host}' 和 Port '{self._port}' 进行连接 (别名: {self.alias})。")
            ...
            logger.warning(f"未提供 URI 或 Host，将尝试使用默认 Host '{self._host}' 和 Port '{self._port}' (别名: {self.alias})。")
            self._connection_info['host'] = self._host
            self._connection_info['port'] = self._port
            self._connection_info['secure'] = self._secure if self._secure is not None else False
        # -- 处理认证信息 --
        if self._token:
            ...
            if hasattr(connections, 'connect') and 'token' in connections.connect.__code__.co_varnames:
                ...
                logger.warning("当前 PyMilvus 版本可能不支持 Token 认证，将忽略 Token 参数。")
        ...
        if hasattr(connections, 'connect') and 'db_name' in connections.connect.__code__.co_varnames:
            ...
            logger.warning(f"当前 PyMilvus 版本可能不支持多数据库，将忽略 db_name='{self._db_name}' 参数。")
        # 合并额外参数，显式参数优先
    ...
    def connect(self) -> None:
        ...
        if self._is_connected:
            ...
            return
        logger.info(f"尝试连接到 Milvus (别名: {self.alias}) 使用参数: {self._connection_info}")
        try:
            connections.connect(
                alias=self.alias,
    ...
    def _ensure_connected(self):
        ...
        if not self.is_connected():
            ...
            self.connect() # 尝试重新连接
        if not self._is_connected:
            raise ConnectionError(f"无法连接到 Milvus (alias: {self.alias})。请检查连接参数和服务器状态。")
    ...
    def create_collection(self, collection_name: str, schema: CollectionSchema, **kwargs) -> Optional[Collection]:
        """
        创建具有给定模式的新集合。
        Args:
    ...
    def drop_collection(self, collection_name: str, timeout: Optional[float] = None) -> bool:
        """
        删除指定的集合。
        Args:
    ...
    def get_collection_stats(self, collection_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = utility.get_collection_stats(collection_name=collection_name, using=self.alias)
    ...
    # --- Data Operations ---
    def insert(self, collection_name: str, data: List[Union[List, Dict]], partition_name: Optional[str] = None, timeout: Optional[float] = None, **kwargs) -> Optional[Any]:
        """
        向指定集合插入数据。
        Args:
        ...
        try:
            ...
            logger.info(f"成功向集合 '{collection_name}' 插入数据。PKs: {mutation_result.primary_keys}")
    ...
    def delete(self, collection_name: str, expression: str, partition_name: Optional[str] = None, timeout: Optional[float] = None, **kwargs) -> Optional[Any]:
        ...
        if not collection:
            ...
            return None
        logger.info(f"尝试从集合 '{collection_name}' 中删除满足条件 '{expression}' 的实体...")
        try:
            ...
            delete_count = mutation_result.delete_count if hasattr(mutation_result, 'delete_count') else 'N/A'
            logger.info(f"成功从集合 '{collection_name}' 发送删除请求。删除数量: {delete_count} (注意: 实际删除需flush后生效)")
            # 考虑是否在这里自动 flush
            # self.flush([collection_name])
            return mutation_result
        except MilvusException as e:
    ...
    # --- Indexing ---
    def create_index(self, collection_name: str, field_name: str, index_params: Dict[str, Any], index_name: Optional[str] = None, timeout: Optional[float] = None, **kwargs) -> bool:
        """
        在指定集合的字段上创建索引。
        Args:
            collection_name (str): 集合名称。
        ...
        try:
            ...
            if not field_exists:
                logger.error(f"字段 '{field_name}' 在集合 '{collection_name}' 的 schema 中不存在。")
                return False
        except Exception as e:
        ...
        # 构建默认索引名
        default_index_name = f"_{field_name}_idx" # PyMilvus 默认可能用 _default_idx 或基于字段
        ...
        try:
            if collection.has_index(index_name=effective_index_name):
                logger.warning(f"集合 '{collection_name}' 的字段 '{field_name}' 上已存在名为 '{effective_index_name}' 的索引。")
                return True # 认为目标已达成
            # 如果没有指定 index_name，也检查一下是否已存在针对该 field 的索引（名称可能未知）
                ...
                for index in indices:
                    if index.field_name == field_name:
                        logger.warning(f"集合 '{collection_name}' 的字段 '{field_name}' 上已存在索引 (名称: {index.index_name})。")
                        return True
        ...
        logger.info(f"尝试在集合 '{collection_name}' 的字段 '{field_name}' 上创建索引 (名称: {effective_index_name})...")
        try:
            ...
            utility.wait_for_index_building_complete(collection_name, index_name=effective_index_name, using=self.alias)
            logger.info(f"成功在集合 '{collection_name}' 的字段 '{field_name}' 上创建并构建索引 (名称: {effective_index_name})。")
            return True
        except MilvusException as e:
            logger.error(f"为集合 '{collection_name}' 字段 '{field_name}' 创建索引失败: {e}")
            return False
        except Exception as e:
            logger.error(f"为集合 '{collection_name}' 字段 '{field_name}' 创建索引时发生意外错误: {e}")
            return False
    ...
    def has_index(self, collection_name: str, index_name: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.error(f"检查集合 '{collection_name}' 的索引 '{index_name or '任意'}' 时出错: {e}")
            return False # 或者抛出异常
        except Exception as e:
            logger.error(f"检查集合 '{collection_name}' 的索引 '{index_name or '任意'}' 时发生意外错误: {e}")
            return False

    def drop_index(self, collection_name: str, field_name: Optional[str] = None, index_name: Optional[str] = None, timeout: Optional[float] = None) -> bool:
            ...
            try:
                ...
                for index in indices:
                    if index.field_name == field_name:
                        ...
                        logger.info(f"找到与字段 '{field_name}' 关联的索引: '{effective_index_name}'。")
                        found = True
                        break
                if not found:
                    logger.warning(f"在集合 '{collection_name}' 中未找到与字段 '{field_name}' 关联的索引，无法删除。")
                    return True # 没有对应索引，认为目标达成
            except Exception as e:
                logger.error(f"查找字段 '{field_name}' 的索引时出错: {e}。无法继续删除。")
                return False
        ...
        try:
            if not collection.has_index(index_name=effective_index_name):
                logger.warning(f"尝试删除不存在的索引（名称: {effective_index_name}）于集合 '{collection_name}'。")
                return True # 认为目标状态已达到
        except IndexNotExistException:
            logger.warning(f"尝试删除不存在的索引（名称: {effective_index_name}）于集合 '{collection_name}'。")
            return True
        except Exception as e:
            logger.warning(f"检查索引 '{effective_index_name}' 是否存在时出错: {e}。将继续尝试删除。")


        logger.info(f"尝试删除集合 '{collection_name}' 上的索引 (名称: {effective_index_name})...")
        try:
            collection.drop_index(index_name=effective_index_name, timeout=timeout)
            logger.info(f"成功删除集合 '{collection_name}' 上的索引 (名称: {effective_index_name})。")
            return True
        except MilvusException as e:
            logger.error(f"删除集合 '{collection_name}' 上的索引 '{effective_index_name}' 失败: {e}")
            return False
        except Exception as e:
            logger.error(f"删除集合 '{collection_name}' 上的索引 '{effective_index_name}' 时发生意外错误: {e}")
            return False

    # --- Search & Query ---
    def load_collection(self, collection_name: str, replica_number: int = 1, timeout: Optional[float] = None, **kwargs) -> bool:
        """
        将集合加载到内存中以进行搜索。
        Args:
        ...
        try:
            ...
            utility.wait_for_loading_complete(collection_name, using=self.alias, timeout=timeout)
            ...
            if "index not found" in str(e).lower():
                logger.error(f"加载失败原因可能是集合 '{collection_name}' 尚未创建索引。")
            return False
        except Exception as e:
            logger.error(f"加载集合 '{collection_name}' 时发生意外错误: {e}")
            return False

    def release_collection(self, collection_name: str, timeout: Optional[float] = None, **kwargs) -> bool:
        """从内存中释放集合。"""
        collection = self.get_collection(collection_name)
        if not collection:
            return False
        ...
        try:
            ...
            logger.warning(f"检查集合 '{collection_name}' 加载状态时出错: {e}。将尝试释放。")
    ...
    def search(
        ...
        **kwargs
        ...
        # 确保集合已加载
        if not self.load_collection(collection_name, timeout=timeout): # 尝试加载，如果失败则退出
            logger.error(f"搜索前加载集合 '{collection_name}' 失败。")
            return None

        logger.info(f"在集合 '{collection_name}' 中搜索 {len(query_vectors)} 个向量 (字段: {vector_field}, top_k: {limit})...")
        try:
    ...
    def query(
        ...
        **kwargs
        ...
        logger.info(f"在集合 '{collection_name}' 中执行查询: '{expression}' (Limit: {effective_limit}, Offset: {offset})...")
        try:
            # 确保 output_fields 包含主键，因为 query 结果默认可能不含（与 search 不同）
            pk_field_name = collection.schema.primary_field.name
            if output_fields and pk_field_name not in output_fields and '*' not in output_fields:
                query_output_fields = output_fields + [pk_field_name]
            elif not output_fields:
                # 如果 None, 尝试获取所有非向量字段 + PK
                query_output_fields = [f.name for f in collection.schema.fields if f.dtype != DataType.FLOAT_VECTOR and f.dtype != DataType.BINARY_VECTOR]
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type:
            logger.error(f"MilvusManager 上下文管理器退出时捕获到异常: {exc_type.__name__}: {exc_val}")
