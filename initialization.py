...
def initialize_config_and_schema(plugin: 'Mnemosyne'):
    ...
    try:
        ...
        fields = [
            FieldSchema(name=PRIMARY_FIELD_NAME, dtype=DataType.INT64, is_primary=True, auto_id=True, description="唯一记忆标识符"),
            FieldSchema(name="personality_id", dtype=DataType.VARCHAR, max_length=256, description="与记忆关联的角色ID"),
            FieldSchema(name="session_id", dtype=DataType.VARCHAR, max_length=72, description="会话ID"), # 增加了长度限制
            FieldSchema(name="content", dtype=DataType.VARCHAR, max_length=4096, description="记忆内容（摘要或片段）"), # 增加了长度限制
            FieldSchema(name=VECTOR_FIELD_NAME, dtype=DataType.FLOAT_VECTOR, dim=embedding_dim, description="记忆的嵌入向量"),
            FieldSchema(name="create_time", dtype=DataType.INT64, description="创建记忆时的时间戳（Unix epoch）")
        ]

        plugin.collection_name = plugin.config.get('collection_name', DEFAULT_COLLECTION_NAME)
        plugin.collection_schema = CollectionSchema(
            fields=fields,
            description=f"长期记忆存储: {plugin.collection_name}",
            primary_field=PRIMARY_FIELD_NAME,
            enable_dynamic_field=plugin.config.get("enable_dynamic_field", False) # 是否允许动态字段
        )

        # 定义索引参数
        plugin.index_params = plugin.config.get("index_params", {
            "metric_type": "L2",       # 默认度量类型
            "index_type": "AUTOINDEX", # 默认索引类型
            "params": {}
        })
        # 定义搜索参数
        plugin.search_params = plugin.config.get("search_params", {
            "metric_type": plugin.index_params.get("metric_type", "L2"), # 必须匹配索引度量类型
            "params": {"nprobe": 10} # IVF_* 的示例搜索参数, AutoIndex 通常不需要
        })

        plugin.output_fields_for_query = plugin.config.get("output_fields", DEFAULT_OUTPUT_FIELDS)
...
def initialize_milvus(plugin: 'Mnemosyne'):
    ...
    try:
        ...
        if not milvus_address:
            raise ValueError("Milvus 'address' (例如 'localhost:19530' 或 URI) 未在配置中指定。")
        ...
        if milvus_address.startswith(("http://", "https://", "unix:")):
            ...
            try:
                ...
                raise ValueError(f"解析 Milvus 地址 '{milvus_address}' 失败: {e}") from e

        # 从配置中添加可选的连接参数
        for key in ["user", "password", "token", "secure", "db_name", "server_pem_path", "server_key_path", "ca_pem_path", "client_pem_path", "client_key_path"]:
            if key in plugin.config:
                connect_args[key] = plugin.config[key]
        
        # 设置连接别名，虽然 MilvusManager 内部可能不直接使用，但保持配置选项
        connect_args["alias"] = plugin.config.get("connection_alias", f"mnemosyne_{plugin.collection_name}") # 使用更有意义的别名

        init_logger.info(f"尝试使用参数连接到 Milvus: { {k: v for k, v in connect_args.items() if k != 'password' and k != 'token'} }") # 不打印敏感信息
        plugin.milvus_manager = MilvusManager(**connect_args)

        if not plugin.milvus_manager or not plugin.milvus_manager.is_connected():
            raise ConnectionError("初始化 MilvusManager 或连接到 Milvus 失败。请检查配置和 Milvus 服务状态。")
...
def setup_milvus_collection_and_index(plugin: 'Mnemosyne'):
    ...
    if plugin.milvus_manager.has_collection(collection_name):
        ...
        if not plugin.milvus_manager.create_collection(collection_name, plugin.collection_schema):
    ...
    if not plugin.milvus_manager.load_collection(collection_name):
        ...
        init_logger.error(f"加载集合 '{collection_name}' 失败。搜索功能可能无法正常工作或效率低下。")
...
def ensure_milvus_index(plugin: 'Mnemosyne', collection_name: str):
    ...
    if not plugin.milvus_manager: return

    try:
        has_vector_index = False
        # 先检查集合是否存在，避免后续操作出错
        if not plugin.milvus_manager.has_collection(collection_name):
            init_logger.warning(f"尝试为不存在的集合 '{collection_name}' 检查/创建索引，跳过。")
            return
        ...
        if collection:
            for index in collection.indexes:
                ...
                if index.field_name == VECTOR_FIELD_NAME:
                    ...
                    # else:
                    init_logger.info(f"在集合 '{collection_name}' 上检测到字段 '{VECTOR_FIELD_NAME}' 的现有索引。")
                    has_vector_index = True
                    break # 找到即可退出循环
        else:
            init_logger.warning(f"无法获取集合 '{collection_name}' 的对象来详细验证索引信息。")

        # 如果没有找到向量索引，则尝试创建
        if not has_vector_index:
            init_logger.warning(f"集合 '{collection_name}' 的向量字段 '{VECTOR_FIELD_NAME}' 尚未创建索引。正在尝试创建...")
            # 使用配置好的索引参数创建索引
            index_success = plugin.milvus_manager.create_index(
                ...
                timeout=plugin.config.get("create_index_timeout", 600) # 增加创建索引的超时设置
            )
            if not index_success:
                init_logger.error(f"为字段 '{VECTOR_FIELD_NAME}' 创建索引失败。搜索性能将受到严重影响。请检查 Milvus 日志。")
                # 根据需要，可以考虑抛出异常
            else:
                init_logger.info(f"已为字段 '{VECTOR_FIELD_NAME}' 发送索引创建请求。索引将在后台构建。")
                # 创建索引后，可能需要等待其构建完成才能获得最佳性能，但通常可以继续运行
                # 可以考虑添加一个检查索引状态的步骤，或者在首次搜索前强制 load

    except Exception as e:
        init_logger.error(f"检查或创建集合 '{collection_name}' 的索引时发生错误: {e}", exc_info=True)
...
def initialize_components(plugin: 'Mnemosyne'):
    ...
    try:
        plugin.context_manager = ConversationContextManager(
            ...
            max_history_length=plugin.config.get("max_history_memory", DEFAULT_MAX_HISTORY)
        )
    ...
    try:
        ...
        try:
            ...
            init_logger.warning("Embedding API 类没有 test_connection 方法，跳过连接测试。")
...
def check_schema_consistency(plugin: 'Mnemosyne', collection_name: str, expected_schema: CollectionSchema):
    """
    ...
    try:
        ...
        for name, expected_field in expected_fields.items():
            ...
            if actual_field.dtype != expected_field.dtype:
                # 特别处理向量类型，检查维度
                is_vector_expected = expected_field.dtype in [DataType.FLOAT_VECTOR, DataType.BINARY_VECTOR]
                is_vector_actual = actual_field.dtype in [DataType.FLOAT_VECTOR, DataType.BINARY_VECTOR]
                ...
                if is_vector_expected and is_vector_actual:
                    ...
                    if expected_dim != actual_dim:
                        warnings.append(f"模式警告：字段 '{name}' 的向量维度不匹配 (预期 {expected_dim}, 实际 {actual_dim})")
                        consistent = False
                elif expected_field.dtype == DataType.VARCHAR and actual_field.dtype == DataType.VARCHAR:
                    ...
                    if expected_len is not None and actual_len is not None and actual_len < expected_len:
                        warnings.append(f"模式警告：字段 '{name}' 的 VARCHAR 长度不足 (预期 {expected_len}, 实际 {actual_len})")
                        consistent = False # 这可能比较严重
                    elif expected_len is not None and actual_len is not None and actual_len > expected_len:
                        warnings.append(f"模式提示：字段 '{name}' 的 VARCHAR 长度大于预期 (预期 {expected_len}, 实际 {actual_len})")
                        # consistent = False # 通常不认为是严重问题
                else:
                    # 其他类型不匹配
                    warnings.append(f"模式警告：字段 '{name}' 的数据类型不匹配 (预期 {expected_field.dtype}, 实际 {actual_field.dtype})")
                    consistent = False
        ...
        for name in actual_fields:
            if name not in expected_fields:
                ...
                if not plugin.collection_schema.enable_dynamic_field:
                    warnings.append(f"模式警告：发现未在配置中定义的字段 '{name}' (且未启用动态字段)")
                    # consistent = False # 是否视为不一致取决于策略

        if not consistent:
            warning_message = f"集合 '{collection_name}' 的 Schema 与当前配置存在潜在不一致:\n - " + "\n - ".join(warnings)
        ...
        init_logger.error(f"检查集合 '{collection_name}' Schema 一致性时发生错误: {e}", exc_info=True)
