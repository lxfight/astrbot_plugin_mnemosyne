...
def validate_session_id(session_id: str) -> bool:
    ...
    if not re.match(pattern, session_id):
        logger.warning(f"session_id 格式验证失败: 包含非法字符")
        return False
    
    if len(session_id) > 255:
        logger.warning(f"session_id 格式验证失败: 长度超过255")
        return False
...
def validate_personality_id(personality_id: str) -> bool:
    ...
    if not re.match(pattern, personality_id):
        logger.warning(f"personality_id 格式验证失败: 包含非法字符")
        return False
    
    if len(personality_id) > 256:
        logger.warning(f"personality_id 格式验证失败: 长度超过256")
        return False
...
def safe_build_milvus_expression(field: str, value: str, operator: str = "==") -> str:
    ...
    if field not in allowed_fields:
        raise ValueError(f"不允许的字段名: {field}. 只允许: {', '.join(allowed_fields)}")
...
def validate_safe_path(file_path: str, base_dir: str, allow_creation: bool = True) -> Path:
    ...
    try:
        ...
        try:
            ...
            raise ValueError(f"路径遍历检测: 路径 '{file_path}' 试图访问基础目录 '{base_dir}' 之外的位置")
...
def normalize_db_path(db_file: Optional[str], default_base_dir: str) -> str:
    """
    规范化数据库文件路径，确保安全性
...
def validate_provider_id(provider_id: Optional[str], available_providers: list) -> tuple[bool, Optional[str]]:
    """
    验证 provider_id 的有效性
    ...
    if not isinstance(provider_id, str):
        return False, f"provider_id 类型无效: 期望 str，得到 {type(provider_id).__name__}"
    
    # 检查格式（只允许字母数字、下划线、连字符）
    if not re.match(r'^[a-zA-Z0-9_-]+$', provider_id):
        return False, f"provider_id 格式无效: 包含非法字符"
    
    # 检查是否在可用列表中
    if available_providers and provider_id not in [p.get('id') for p in available_providers if isinstance(p, dict) and 'id' in p]:
...
def sanitize_error_message(error_msg: str, remove_paths: bool = True, remove_values: bool = True) -> str:
    """
    清理错误消息，移除敏感信息
