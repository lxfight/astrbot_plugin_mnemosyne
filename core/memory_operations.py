...
from .constants import (
    VECTOR_FIELD_NAME, PRIMARY_FIELD_NAME, DEFAULT_TOP_K,
    DEFAULT_MILVUS_TIMEOUT, DEFAULT_PERSONA_ON_NONE
)
...
async def handle_query_memory(plugin: 'Mnemosyne', event: AstrMessageEvent, req: ProviderRequest):
        ...
        for record in reversed(req.contexts):
            if record.get("role") == "user":
                ...
                if i > contexts_memory_len:
                    ...
                    # 使用非贪婪匹配移除标记及其内容
                    clean_content = re.sub(r'<Mnemosyne>.*?</Mnemosyne>\n?', '', raw_content, flags=re.DOTALL).strip()
    ...
    try:
        # --- 获取会话和人格信息 ---
        session_id = await plugin.context.conversation_manager.get_curr_conversation_id(event.unified_msg_origin)
        conversation = await plugin.context.conversation_manager.get_conversation(event.unified_msg_origin, session_id)
        ...
        if not persona_id or persona_id == "[%None]":
            ...
            if not persona_id:
                 logger.warning(f"当前会话 (ID: {session_id}) 及全局均未配置人格，将使用占位符 '{DEFAULT_PERSONA_ON_NONE}' 进行记忆操作（如果启用人格过滤）。")
        ...
        if plugin.context_manager:
            ...
            memory_summary = plugin.context_manager.add_message(session_id=session_id, role="user", content=memory_summary_input)

            if memory_summary:
                logger.info(f"短期记忆达到阈值，触发后台总结任务 (Session: {session_id[:8]}...)")
                # 确保 persona_id 正确传递 (可能是 None 或占位符)
                asyncio.create_task(handle_summary_long_memory(plugin, persona_id, session_id, memory_summary))
        else:
        ...
        try:
            ...
            # 组合过滤器
            search_expression = " and ".join(filters) if filters else "" # 如果没有过滤器，则为空字符串
            ...
            logger.info(f"开始在集合 '{collection_name}' 中搜索相关记忆 (TopK: {top_k}, Filter: '{search_expression or '无'}')")
            ...
            # 4. 处理搜索结果
            if not search_results or not search_results[0]: # search 返回 List[SearchResult], SearchResult 可能为空
                ...
                detailed_results = [hit.entity.to_dict()['entity'] for hit in hits if hit.entity] # 假设 to_dict 返回 {'entity': {...}} 结构
        ...
        if detailed_results:
            ...
            long_memory_prefix = plugin.config.get("long_memory_prefix", "<Mnemosyne>长期记忆片段：")
            ...
            for result in detailed_results:
                ...
                memory_entry_format = plugin.config.get("memory_entry_format", "- [{time}] {content}")
            ...
            injection_method = plugin.config.get("memory_injection_method", "append_to_prompt") # append_to_prompt | prepend_to_prompt | system_prompt
            ...
            #     req.prompt = (req.prompt or "") + "\n\n" + long_memory
                 ...
                 logger.warning(f"未知的记忆注入方法 '{injection_method}'，将默认追加到用户 prompt。")
                 req.prompt = (req.prompt or "") + "\n\n" + long_memory
        else:
...
async def handle_on_llm_resp(plugin: 'Mnemosyne', event: AstrMessageEvent, resp: LLMResponse):
    """
    处理 LLM 响应后的逻辑。
    ...
    try:
        ...
        memory_summary = plugin.context_manager.add_message(session_id=session_id, role="assistant", content=llm_response_text)

        if memory_summary:
            logger.info(f"短期记忆达到阈值（LLM响应后），触发后台总结任务 (Session: {session_id[:8]}...)")
...
async def handle_summary_long_memory(plugin: 'Mnemosyne', persona_id: Optional[str], session_id: str, memory_text: str):
    """
    ...
    try:
        # 1. 请求 LLM 进行总结
        long_memory_prompt = plugin.config.get("long_memory_prompt", "请将以下多轮对话历史总结为一段简洁、客观、包含关键信息的长期记忆条目:")
        # 添加上下文，指示 LLM 总结的是对话历史
        summary_context = [{"role": "system", "content": long_memory_prompt}, {"role": "user", "content": memory_text}]
        
        # 获取总结用的LLM配置，允许与主LLM不同
        summary_llm_config = plugin.config.get("summary_llm_config", {}) # 例如 {"model": "gpt-3.5-turbo", "max_tokens": 200}
        
        logger.debug(f"请求 LLM 总结短期记忆，提示: '{long_memory_prompt[:50]}...', 内容长度: {len(memory_text)}")
        
        # 使用 text_chat 并传入可能的配置覆盖
        llm_response = await llm_provider.text_chat(
            prompt=memory_text, # 主内容放在 prompt
            contexts=[{"role": "system", "content": long_memory_prompt}], # 引导指令放在 system context
        ...
        effective_persona_id = persona_id if persona_id else plugin.config.get("default_persona_id_on_none", DEFAULT_PERSONA_ON_NONE)
        ...
        # 5. 执行插入操作
        logger.info(f"准备向集合 '{collection_name}' 插入 1 条总结记忆 (Persona: {effective_persona_id}, Session: {session_id[:8]}...)")
        ...
        if mutation_result and mutation_result.insert_count > 0:
            ...
            if flush_after_insert:
                try:
                    logger.debug(f"正在刷新 (Flush) 集合 '{collection_name}' 以确保记忆立即可用...")
                    plugin.milvus_manager.flush([collection_name])
                    logger.debug(f"集合 '{collection_name}' 刷新完成。")
                except Exception as flush_err:
                    logger.error(f"刷新集合 '{collection_name}' 时出错: {flush_err}", exc_info=True)
        else:
            # 插入失败的原因可能需要看 Milvus 日志
            logger.error(f"插入总结记忆到 Milvus 失败。MutationResult: {mutation_result}. LLM 回复: {summary_text[:100]}...")
